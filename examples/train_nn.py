#!/usr/bin/env python3
"""Train a multiscale LODE neural network potential based on energies.

The script will write an extended xyz file where the info section containes keys for
`predicted_energy`, `predicted_binding` containing the predicted energies. Additionally,
a key called `split` indicated for each frame wether it was used for training or for
testing."""
...
parser.add_argument(
    ...
    "--dataset",
    type=str,
    default=f"{dataset}/charge-charge.xyz",
    help="dataset file",
)
parser.add_argument("-p", "--prefix", type=str, default="test", help="The prefix")
parser.add_argument("-r", "--rcut", type=float, default=4.0, help="train cutoff value")
...
parser.add_argument(
    ...
    type=str,
    default="01",
    help='Potential exponents to be considered, e.g. "016"',
)
...
all_frames = ase.io.read(args.dataset, "::" + str(STRIDE))
...
try:
    ...
    metatensor.save(PREFIX + "-descriptor_soap.npz", descriptor_soap)
...
try:
    ...
    metatensor.save(PREFIX + "-descriptor_lode1.npz", descriptor_lode1)
...
try:
    ...
    metatensor.save(PREFIX + "-descriptor_lode3.npz", descriptor_lode3)
...
try:
    ...
    metatensor.save(PREFIX + "-descriptor_lode6.npz", descriptor_lode6)
...
for epoch in range(n_epochs):
    ...
    # A very simple manual dataloder
